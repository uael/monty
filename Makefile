...
# cause. Point it at the interpreter behind this workspace's virtualenv, and
# expand to nothing when there is none, so pyo3's own search still applies.
# `build.rs` fails the build with an explanation whenever what it finds is too
# old. The base interpreter rather than the virtualenv itself, because an
# embedded interpreter reads its standard library from the prefix it was built
# with and a virtualenv has none. `PYTHONHOME` names that prefix at run time,
# which a relocatable build (every interpreter uv manages) needs because the
# prefix it was built with is not where it now lives. `LC_ALL` is there because
# an embedded interpreter reads a test case with the locale's encoding, and a
# machine whose locale is `C` then refuses a fixture holding an em dash.
DATATEST_LOCALE = $(shell locale -a 2>/dev/null | grep -i -m1 -E '^(C|en_US)\.(utf-?8|UTF-?8)$$' || echo C.UTF-8)
DATATEST_PYTHON = LC_ALL=$(DATATEST_LOCALE) $(shell p=$$(uv run --no-sync python -c 'import sys; print(sys._base_executable)' 2>/dev/null); test -n "$$p" && echo PYO3_PYTHON=$$p PYTHONHOME=$$(dirname $$(dirname $$p)))
