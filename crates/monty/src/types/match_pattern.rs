...
/// two-character string apart. What is left is the builtin sequences Monty has.
pub(crate) fn is_match_sequence(subject: &Value, vm: &mut VM<'_>) -> bool {
    matches!(
        subject.py_type(vm),
...
pub(crate) fn is_match_mapping(subject: &Value, vm: &mut VM<'_>) -> bool {
    matches!(subject.py_type(vm), Type::Dict | Type::DefaultDict | Type::Counter)
}
