...
fn call_rebound(vm: &mut VM<'_>, args: ArgValues) -> RunResult<Value> {
    ...
    let source = namespace(source, "source", vm)?;
    let target = namespace(target, "target", vm)?;
    copy::rebound(x, source, target, memo, vm)
}

/// The dict a namespace argument is, borrowed, or the `TypeError` for what is
/// no dict.
fn namespace(value: &Value, which: &str, vm: &mut VM<'_>) -> RunResult<HeapId> {
    match value {
        Value::Ref(id) if matches!(vm.heap.read(*id), HeapReadOutput::Dict(_)) => Ok(*id),
        other => Err(ExcType::type_error(format!(
            "rebound() {which} must be a dict, not {}",
